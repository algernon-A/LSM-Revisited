...
namespace LoadingScreenMod
{
    public sealed class PrefabLoader : DetourUtility<PrefabLoader>
    {
        ...
        public static bool QueueLoadingAction(LoadingManager __instance, IEnumerator action)
        {
            ...
            while (!Monitor.TryEnter(LoadingScreenModRevisited.LevelLoader.s_loadingLock, SimulationManager.SYNCHRONIZE_TIMEOUT))
            {
            }
            try
            {
                ...
                LoadingScreenModRevisited.LevelLoader.s_mainThreadQueue.Enqueue(action);
                if (LoadingScreenModRevisited.LevelLoader.s_mainThreadQueue.Count < 2)
                {
            ...
            {
                Monitor.Exit(LoadingScreenModRevisited.LevelLoader.s_loadingLock);
            }
